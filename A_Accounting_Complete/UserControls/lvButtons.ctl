...
Private Type ButtonProperties
    ...
    bBackHoverEnd As Long                   '/ button back color end when mouse hovers
...
Private adjHoverColorEnd As Long     '/ cached hover backcolor end
...
Public Property Let HoverBackColorEnd(nColor As OLE_COLOR)

' Changes the backcolor when mouse is over the button
' Changing this property will affect the type of HoverLock

If myProps.bBackHoverEnd = nColor Then Exit Property
myProps.bBackHoverEnd = nColor
If nColor <> curBackColor Then
    If myProps.bLockHover = lv_LockTextandBackColor Then
        myProps.bLockHover = lv_LockTextColorOnly
    Else
...
Public Property Get HoverBackColorEnd() As OLE_COLOR
HoverBackColorEnd = myProps.bBackHoverEnd
End Property
...
Dim mousePt As POINTAPI     '/, cRect As RECT Removed by ellis (variable not used).
...
'/ Dim tBrush As Long, tPen As Long Removed by Ellis (Variable not used)
...
Case "BackColor"
    ...
    '/ Added by Ellis.
    adjHoverColorEnd = ConvertColor(myProps.bBackHoverEnd)
    If myProps.bBackStyle = 7 Then
        ...
        '/ Added by Ellis.
        adjHoverColorEnd = ShadeColor(adjHoverColorEnd, &H1F, False)
    ...
    ElseIf myProps.bBackStyle = 2 Then
        ...
        '/ Added by Ellis.
        adjHoverColorEnd = ShadeColor(adjHoverColorEnd, &H30, True)
...
'/Modified by Ellis - add bHoverColorEnd.
Private Sub DrawButtonBackground(bColor As Long, ActiveStatus As Integer, ActiveRegion As Integer, _
                Optional bGradientColor As Long = -1, Optional bHoverColor As Long = -1, _
                Optional bHoverColorEnd As Long = -1)
...
If myProps.bGradient And myProps.bValue = False And myProps.bShape < lv_CustomFlat Then
    ...
    If bTimerActive = True And ((myProps.bStatus And 6) = 6) = False And _
        ...
        DrawGradient bHoverColorEnd, bHoverColor
    Else
...
Dim i As Integer, xColorRef As Integer  '/, J As Integer Removed by Ellis (Variable not used).
...
'/ Dim tRgn As Long, hBrush As Long Removed by Ellis (variable not used)
...
Dim lColor As Long '/, fRect As RECT, I As Integer Removed by Ellis (Variable not used)
...
Dim backShade As Long, liteShade As Long '/ darkShade As Long, midShade As Long Removed by Ellis (Variable not used).
Dim i As Integer, lColor As Long
...
'/Modified by Ellis - add adjHoverColorEnd
DrawButtonBackground backShade, ActiveStatus, lastClipRgn, _
            ConvertColor(myProps.bGradientColor), adjHoverColor, adjHoverColorEnd
